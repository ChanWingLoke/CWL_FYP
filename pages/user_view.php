...
if ($id === '') {
  // If no ID is provided, show an error and stop
...
if (isset($db) && $db instanceof PDO) {
  try {
    $stmt = $db->prepare("SELECT `id`,`username`,`user_role`,`added_date`,`last_update_at`,`photo` FROM `user` WHERE `id` = :id LIMIT 1");
    $stmt->execute([':id' => $id]);
  ...
  <div class="content-header">
    <div class="container-fluid d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <?php if ($isAdmin && $u): ?>
          <a class="btn btn-sm btn-primary" href="index.php?page=user_edit&id=<?= urlencode($u['id']) ?>">
          ...
          <div class="row">
            <div class="col-md-3 text-center">
              <?php if (!empty($u['photo'])): ?>
                <img src="<?= h($u['photo']) ?>" alt="User Photo" class="img-circle elevation-2" style="height:120px;width:120px;object-fit:cover;">
              <?php else: ?>
                <div class="d-flex align-items-center justify-content-center bg-light img-circle elevation-1" style="height:120px;width:120px;margin:0 auto;">
            ...
            <div class="col-md-9">
              <dl class="row mb-0 mt-3 mt-md-0">
                <dt class="col-sm-3">ID</dt>
                <dd class="col-sm-9"><?= h($u['id']) ?></dd>
                ...
                <dd class="col-sm-9">
                  ...
                  <?php else: ?>
                    <span class="badge badge-secondary"><?= h(ucfirst($u['user_role'] ?: 'user')) ?></span>
                  <?php endif; ?>
                </dd>

                <dt class="col-sm-3">Date Added</dt>
                <dd class="col-sm-9"><?= h($u['added_date']) ?></dd>

                <dt class="col-sm-3">Last Updated</dt>
                <dd class="col-sm-9"><?= h($u['last_update_at'] ?? '—') ?></dd>
              </dl>
            </div>
          </div>
        </div>
